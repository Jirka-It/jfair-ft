...
export class AppInlineMenuComponent {
    ...
    onClick(event) {
        this.pagesMain.onInlineMenuClick(event, this.key);
        event.preventDefault();
    }

    get isTooltipDisabled() {
        return !(this.pagesMain.isSlim() && !this.pagesMain.isMobile());
    }

    get tabIndex() {
        return !this.pagesMain.inlineMenuActive  ? '-1' : null;
    }

    isHorizontalActive() {
       return this.pagesMain.isHorizontal() && !this.pagesMain.isMobile();
    }
}
